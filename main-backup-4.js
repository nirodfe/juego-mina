...
class GameScene extends Phaser.Scene {
    ...
    create() {
        ...
        for (let x = 0; x < gridSize; x++) {
            for (let y = 0; y < gridSize; y++) {
                if (this.grid[x][y].type === 'tierra') {
                    // Dibujar una celda de tierra
                    this.grid[x][y].sprite = this.add.image(x * this.tileSize, y * this.tileSize, 'tierra')
                        .setOrigin(0)
                        .setDisplaySize(this.tileSize, this.tileSize);
                } else if (this.grid[x][y].type === 'piedra') {
                    // Dibujar una celda de piedra
    ...
    triggerEffect(type) {
        if (type === 'tierra') {
            ...
            this.player.setTint(0x8B4513); // Cambiar temporalmente a un color marrón
            this.time.delayedCall(200, () => this.player.clearTint()); // Eliminar el efecto después de 200ms
        } else if (type === 'piedra') {
            console.log('Efecto: Minero pica piedra');
            this.player.setTint(0x444444); // Cambiar temporalmente a un color gris
            this.time.delayedCall(200, () => this.player.clearTint()); // Eliminar el efecto después de 200ms
        }
    }
}
